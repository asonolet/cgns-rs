...
fn main() {
    ...
    let zone = base
        .create_zone_structured("Grid2D", &[5, 4, 4, 3])
        .unwrap();
    ...
    let sol = zone
        .write_solution("Solution", GridLocation::Vertex)
        .unwrap();
    ...
    let zone2 = base2
        .create_zone_structured("Volume", &[4, 3, 5, 3, 2, 4, 0, 0, 0])
        .unwrap();
    let nverts = 60usize;
    ...
    zone3
        .write_section("TriSection", ElementType::Tri3, 1, 2, 0, &tri_conn)
        .unwrap();
    // Hex: 1 hex
    let hex_conn: Vec<i64> = (1..=8).collect();
    zone3
        .write_section("HexSection", ElementType::Hexa8, 3, 3, 0, &hex_conn)
        .unwrap();
    // Tet: 1 tet
    let tet_conn: Vec<i64> = vec![1, 2, 3, 4];
    zone3
        .write_section("TetSection", ElementType::Tetra4, 4, 4, 0, &tet_conn)
        .unwrap();
